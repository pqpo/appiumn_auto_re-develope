...
# -*- coding: utf-8 -*-
import sys
sys.path.append("E:\\github\\appiumn_auto-master")
import datetime
import xlsxwriter
import time
import unittest
...
from testCase.android.loginCase import TestLogin
...
def get_common_report(start_test_time, endtime, starttime, device):
    get_report_collect(start_test_time, endtime, starttime, device)
    get_report_init()
    get_report_info()
...
def runnerPool():
    ...
    for i in range(0, len(ga["appium"])):
        ...
        t["resetApp"] = False
        if 'resetApp' in item:
            t["resetApp"] = item["resetApp"] == 1
        t["deviceName"] = item["devices"]
        t["platformVersion"] = phoneBase.get_phone_info(devices=item["devices"])["release"]
        t["platformName"] = item["platformName"]
        t["port"] = item["port"]
        l_pool.append(t)
...
def runnerCaseApp(l_devices):
    ...
    suite.addTest(TestInterfaceCase.parametrize(TestLogin, device=l_devices[0]))
    unittest.TextTestRunner(verbosity=2).run(suite)
    endtime = datetime.datetime.now()
    get_common_report(start_test_time, endtime, starttime, l_devices[0])
    report()
